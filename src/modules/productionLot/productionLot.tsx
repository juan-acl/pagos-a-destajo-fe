...
const fetcher = () =>
  api.get<ApiEnvelope<ProductionLot[]>>("/production-lot").then((response) => response.data.data);
  ...
  const create = useMutation({
    mutationFn: (payload: ProductionLotForm) =>
      api.post("/production-lot", {
        ...
        revisionProduccionId:
          payload.revisionProduccionId === "" ? undefined : Number(payload.revisionProduccionId),
      }),
          ...
          <div style={s.card}>
            ...
            <form onSubmit={submit}>
              <div style={s.grid}>
                <label style={s.label}>
                  Número de lote *
                  <input name="numeroLote" value={form.numeroLote} onChange={change} required style={s.input} />
                </label>
                <label style={s.label}>
                ...
                <label style={s.label}>
                  Fecha envío *
                  <input name="fechaEnvio" type="date" value={form.fechaEnvio} onChange={change} required style={s.input} />
                </label>
                <label style={s.label}>
                  Estado
          ...
          <div style={s.tableWrap}>
            {isLoading ? (
              ...
              <table style={s.table}>
                <thead>
                  <tr style={s.thead}>
                    {["ID", "Lote", "Piezas aprobadas", "Fecha envío", "Estado", "Revisión ID", "Acciones"].map((h) => (
                      <th key={h} style={s.th}>
                        {h}
                      </th>
                    ))}
